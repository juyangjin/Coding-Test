...
def create_readme_if_not_exists():
    """
    README.md 파일이 존재하지 않으면 새로 생성합니다.
    """
    readme_path = "README.md"
    if not os.path.exists(readme_path):  # README.md가 없다면
        print("README.md not found. Creating a new one.")
        with open(readme_path, "w") as f:
            f.write(HEADER)
        return False  # 새로 만들었으므로 업데이트가 필요 없음
    return True  # 이미 존재하면 그대로
...
if __name__ == "__main__":
    ...
    """
    if create_readme_if_not_exists() and generate_readme():
        commit_and_push()
    else:
